        ...
        try:
            ...
            if not STATE["hidden_data"]:
                ...
                if data:
                    ...
                    # Nếu domain allowed -> Restore ngay để paste dễ dàng
                    if is_allowed:
                        restore_clipboard(d_type, data)
            ...
            if is_allowed:
                # Domain xịn -> Luôn restore để user paste nhiều lần thoải mái
                # Kiểm tra xem có data mới được copy không (không phải do paste)
                current_count = get_pasteboard_change_count()
                if current_count != STATE["last_change_count"]:
                    # Có thể user đã copy cái mới -> check
                    d_type_new, data_new = get_and_clear_clipboard()
                    if data_new:
                        new_hash = get_content_hash(data_new)
                        old_hash = get_content_hash(STATE["hidden_data"])
                        # Chỉ update nếu thực sự là data mới (khác hoàn toàn)
                        if new_hash != old_hash and new_hash != STATE.get("safe_hash"):
                            # Data hoàn toàn mới -> Update state và check
                            STATE["last_change_count"] = current_count
                            STATE["source_app"] = app_name
                            STATE["hidden_data"] = data_new
                            STATE["hidden_type"] = d_type_new
                            STATE["content_type"] = None
                            restore_clipboard(d_type_new, data_new)
                            threading.Thread(target=async_analysis_universal, args=(data_new, d_type_new)).start()
                            time.sleep(0.2)
                            continue
                        else:
                            # Cùng data hoặc safe data -> restore lại
                            STATE["last_change_count"] = current_count
                            if new_hash == STATE.get("safe_hash"):
                                restore_clipboard(d_type_new, data_new)
                            else:
                        ...
                        # Clipboard empty -> restore lại hidden_data
                        STATE["last_change_count"] = current_count
                
                # Luôn restore để đảm bảo có thể paste nhiều lần thoải mái
                restore_clipboard(STATE["hidden_type"], STATE["hidden_data"])
            else:
                # Domain lởm -> Xóa
                clear_clipboard()
...
def on_paste_attempt():
    ...
    try:
        ...
        # Kiểm tra domain allowed
        is_domain_allowed_now = False
        if app_name in BROWSER_APPS:
            if STATE.get("browser_allowed", False): 
                is_domain_allowed_now = True
            else:
                # Fallback: Nếu biến chưa kịp update thì check thủ công
                url = get_active_browser_url(app_name)
                if is_domain_allowed(url):
                    is_domain_allowed_now = True

        # Nếu domain allowed -> hiện alert (cả CODE và TEXT), không gửi email, không debounce
        if is_domain_allowed_now:
            if STATE["hidden_data"]:  # Có dữ liệu đang được giữ
                source_app = STATE.get("source_app", "Unknown")
                print(f"⚠️ [PASTE WARNING] Paste in allowed domain: {app_name}")
                show_alert(app_name, source_app, skip_debounce=True)
            return  # Return để không chặn paste (giữ nguyên logic paste)
