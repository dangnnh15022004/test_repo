...
# ==============================
def create_git_email_helper():
    """Tạo script Python helper để git hook gọi gửi email."""
    helper_script = os.path.join(HOOKS_DIR, "dlp_git_email.py")
    try:
        # Tạo script Python để gửi email từ git hook
...
# Load env từ file .env trong BASE_DIR
BASE_DIR = "{BASE_DIR}"
...
def send_email_git_push(repo_url):
    ...
    try:
        ...
        server.quit()
        print("📧 [EMAIL] Git Push alert sent", file=sys.stderr)
    except Exception as e:
...
'''
        ...
        print(f"Error creating git email helper: {e}")
        return None
...
def setup_git_firewall():
    ...
    try:
        ...
        # 2. Lấy đường dẫn đầy đủ của dlp_agent_mac.py
        if getattr(sys, 'frozen', False):
            # Nếu là executable đã đóng gói
...
# Gửi email cảnh báo khi push bị chặn - gọi trực tiếp dlp_agent_mac.py
if [ -f "{agent_script_escaped}" ]; then
    python3 "{agent_script_escaped}" --git-push-alert "$url" > /tmp/dlp_git_email.log 2>&1 &
else
    echo "⚠️ [DLP] Agent script not found: {agent_script_escaped}" >&2
fi
